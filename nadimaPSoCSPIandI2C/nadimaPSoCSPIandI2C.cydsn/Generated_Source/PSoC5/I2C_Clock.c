...
void I2C_Clock_Start(void) 
{
    ...
    I2C_Clock_CLKEN |= I2C_Clock_CLKEN_MASK;
	I2C_Clock_CLKSTBY |= I2C_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: I2C_Clock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I2C_Clock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    I2C_Clock_CLKEN &= (uint8)(~I2C_Clock_CLKEN_MASK);
	I2C_Clock_CLKSTBY &= (uint8)(~I2C_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: I2C_Clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I2C_Clock_StopBlock(void) 
{
    if ((I2C_Clock_CLKEN & I2C_Clock_CLKEN_MASK) != 0u)
    {
...
#if defined(I2C_Clock__CFG3)
        CLK_DIST_AMASK = I2C_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = I2C_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* I2C_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(I2C_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(I2C_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: I2C_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I2C_Clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        I2C_Clock_CLKSTBY &= (uint8)(~I2C_Clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: I2C_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I2C_Clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = I2C_Clock_GetSourceRegister();
    uint16 oldDivider = I2C_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = I2C_Clock_CLKEN & I2C_Clock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((I2C_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: I2C_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 I2C_Clock_GetDividerRegister(void) 
{
    return CY_GET_REG16(I2C_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: I2C_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I2C_Clock_SetModeRegister(uint8 modeBitMask) 
{
    I2C_Clock_MOD_SRC |= modeBitMask & (uint8)I2C_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: I2C_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I2C_Clock_ClearModeRegister(uint8 modeBitMask) 
{
    I2C_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(I2C_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: I2C_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 I2C_Clock_GetModeRegister(void) 
{
    return I2C_Clock_MOD_SRC & (uint8)(I2C_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: I2C_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I2C_Clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = I2C_Clock_GetDividerRegister();
    uint8 oldSrc = I2C_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        I2C_Clock_MOD_SRC |= CYCLK_SSS;
        I2C_Clock_MOD_SRC =
            (I2C_Clock_MOD_SRC & (uint8)(~I2C_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: I2C_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 I2C_Clock_GetSourceRegister(void) 
{
    return I2C_Clock_MOD_SRC & I2C_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: I2C_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I2C_Clock_SetPhaseRegister(uint8 clkPhase) 
{
    I2C_Clock_PHASE = clkPhase & I2C_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: I2C_Clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 I2C_Clock_GetPhaseRegister(void) 
{
    return I2C_Clock_PHASE & I2C_Clock_PHASE_MASK;
}
